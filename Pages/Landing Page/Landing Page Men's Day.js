document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCartIcon() {
      const cartIcon = document.querySelector('a[href="../Cart Page/cart.html"]');
      ...
      if (itemCount > 0) {
        ...
        cartIcon.querySelector('.cart-count').textContent = itemCount;
      } else if (cartIcon.querySelector('.cart-count')) {
        cartIcon.querySelector('.cart-count').remove();
      }
    }
    
    // Update cart icon when cart changes
    function saveCartToLocalStorage() {
      localStorage.setItem('cart', JSON.stringify(cart));
      updateCartIcon();
    }
